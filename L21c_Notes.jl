...
function linear_classifier_min_norm(a_vectors, b_vectors)
    ...
    @objective(model, Min, sum(w[i]^2 for i in 1:d))

    # Add constraints for a_vectors
    for a in a_vectors
        @constraint(model, a' * w + beta >= 1)
    end

    # Add constraints for b_vectors
    for b in b_vectors
        @constraint(model, b' * w + beta <= -1)
    end
    ...
    return optimal_w, optimal_beta
end

# Solve using the min norm approach
w_min_norm, beta_min_norm = linear_classifier_min_norm(a_vectors, b_vectors)
println("Optimal w (min norm): ", w_min_norm)
println("Optimal beta (min norm): ", beta_min_norm)

# Plot the decision boundary for the min norm solution
y_vals_min_norm = (-beta_min_norm .- w_min_norm[1] .* x_vals) ./ w_min_norm[2]
