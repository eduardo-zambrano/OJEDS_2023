...
## Speed Issues
# Source: https://web.eecs.umich.edu/~fessler/course/551/julia/tutor/03-diag.html

D = Diagonal(10:10:50)

# The data type of the output of Diagonal is not an ordinary 2D Array but rather a special Diagonal type that behaves like a matrix but is stored efficiently.
...
#    "matrix" is quoted because it is not a standard 2D Array in Julia, but rather a special SparseMatrix type that efficiently stores only the nonzero elements and their array indexes. 
...
elapsed_nanoseconds_M = elapsed_seconds_M * 1e9
